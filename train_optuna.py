...
def get_args_parser():
    ...
    parser.add_argument('--optuna_name', default=None, type=str)
    parser.add_argument('--optuna_resume', default=False, action='store_true')
    parser.add_argument('--optuna_ntrials', default=10, type=int)
...
def main_training(args):
    ...
    if args.global_rank == 0 and args.use_wandb:
        ...
        wandb.init(
            ...
            mode=os.environ['WANDB_MODE'] if 'WANDB_MODE' in os.environ else 'offline'
        )
...
def objective(trial):
    ...
    if args.global_rank == 0:
        ...
        args.weight_decay = trial.suggest_float('weight_decay', 0., 0.5)
    ...
    # Training
    max_accuracy = main_training(args)
    
    # Synchronize before ending training
    torch.distributed.barrier()
    return max_accuracy
...
if __name__ == '__main__':
    ...
    # Initialize distributed environment for optuna   
    utils.init_distributed_mode(args)
    
    if args.global_rank == 0:
        # First check the study name
        if args.optuna_name is None:
            args.optuna_name = f"{args.model}_Optuna"
        
        if not args.optuna_resume:
            # If the study already exists, remove the study
            if os.path.exists(f"{args.optuna_name}.pkl"):
                os.remove(f"{args.optuna_name}.pkl")
                print(f"Existing sampler status {args.optuna_name}.pkl has been deleted.")
                
            # If the study already exists, remove the study
            if os.path.exists(f"{args.optuna_name}.db"):
                os.remove(f"{args.optuna_name}.db")
                print(f"Existing study {args.optuna_name}.db has been deleted.")
        
        # Get the study storage
        storage_url = f"sqlite:///{args.optuna_name}.db"
        print(f"Create or resume study {args.optuna_name} from {storage_url}")
        ...
        study = optuna.create_study(
            ...
            load_if_exists=args.optuna_resume,
